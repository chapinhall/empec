...
This code pulls together data from the [2020 Census redistricting files](https://api.census.gov/data/2020/dec/pl.html), 2010 SF1 Census files, and `r glue('{acs5_year-4}-{acs5_year}')` ACS 5-year to get population by age group to use in projecting population counts for young children through 2022.
...
```{r pull data from 2020 PL 94-171 Redistricting Data}
#tract level
#population
...
var_pl_2020 <- load_variables(2020, dataset = "pl")
...
pop_tract_2020_redis <- 
  get_decennial(
    geography = "tract",
    variables = var_pl_2020$name,
    year = 2020,
    sumfile = "pl",
    state = my_state_fip,
    county = county_arg,
    geometry = FALSE) %>%
  merge(var_pl_2020 %>% select(-label, -concept),
        by.x = "variable",
        by.y = "name",
        all = TRUE)
...
pop_tract_2020_dhc_test <-
  ...
  aggregate(value ~ GEOID, data = pop_tract_2020_dhc, FUN = sum) %>%
  rename(total_0_17_2020_dhc = value)

comp <- 
  merge(pop_tract_2020_redis, 
        pop_tract_2020_dhc_test, 
        by = "GEOID",
...
comp_dhc_redis_acs5 <- 
  merge(pop_age_groups_2020_dhc,
        pop_age_groups_2020 %>% rename(value = est_2020),
        by = c("GEOID", "agegroup"),
...
crosswalk <-
  ...
  mutate(statefip = as.character(substr(GEOID20, 1,2))) %>%
  filter(statefip == {my_state_fip})

# In the future, crosswalk may be downloaded using IPUMS API
if (FALSE) {
  bg2010_tr2020 <- define_extract_nhgis(
  description = "bg2010_tr2020",
  datasets = ds_spec(
    "bg2010_tr2020",
    data_tables = "",
    geog_levels = ""
  ...
  submit_extract(bg2010_tr2020, api_key = {ipums_key})
  ipums.wait_for_extract(bg2010_tr2020)
  ipums.download_extract(bg2010_tr2020)
}
...
```{r temporarily use dfss pop est}
pop_0017_2010_2023_tract <- read.csv("H:/DFSS Community Assessment/Population_Poverty/Population estimates 2022-2023/tract_age_estimates/pop_0017_2010_2023_tract.csv")

ests_with_forecast_wide_2 <- 
  pop_0017_2010_2023_tract %>%
  mutate(GEOID = TRACT20) %>%
  select(-TRACT20, -TRACTCE20, -cca, -cca_name, -race, -agegroup) %>%
  select(GEOID, age, everything()) %>%
  arrange(GEOID, age)
...
ests_with_forecast_long <- 
  ...
  mutate(GEOID = as.character(GEOID),
         year = str_extract(name, "\\d+") %>% as.numeric(),
         age = case_when(age %in% 0:2   ~ "age_0to2_count",
                         age %in% 3:5   ~ "age_3to5_count",
                         age %in% 6:12  ~ "age_6to12_count",
                         age %in% 13:17 ~ "age_13to17_count")) %>% 
  group_by(age, GEOID, year) %>%
  summarise(value = sum(value))
```


```{r reapportion population estimates to 2010 census geographies}
if (base_year <= 2021) {
  rel_10_20 <- 
    ...
    filter(str_detect(GEOID_TRACT_10, glue("^{my_state_fip}")), 
  ...
  ests_with_forecast_long <- 
    ...
    merge(rel_10_20 %>% select(GEOID_TRACT_20, GEOID = GEOID_TRACT_10, part_of_20),
          by = "GEOID_TRACT_20",
          all.x = TRUE) %>% 
...
```{r reshape temporary population estimates}
pop_by_age_2 <- 
  ests_with_forecast_long %>% 
  pivot_wider(names_from  = "age",
              values_from = "value") %>% 
...
# Duplicate year 2022 and add it to the current data as year 2023
# This is done--temporarily--given the concern around using volatile recent
# trends as a basis for forecasting further ahead
pop_by_age_2_toadd <- 
  pop_by_age_2 %>% 
  filter(year == 2022) %>% 
  mutate(year = 2023)
...
pop_by_age_2 <- 
  bind_rows(pop_by_age_2,
            pop_by_age_2_toadd)

```


```{r compare population estimates of 2022 as of May 2022 and new update with dfss}
comp_pop <- 
  pop_by_age_2 %>% 
  filter(year == 2022) %>% 
  merge(pop_by_age %>% select(GEOID, age_0to2_count, age_3to5_count),
        by = "GEOID",
        #all = TRUE,
        suffixes = c(".pop_dfss", ".pop_may2023"))

ggplot(comp_pop,
       aes(x = age_0to2_count.pop_dfss,
           y = age_0to2_count.pop_may2023)) +
  geom_point(alpha = 0.3) + 
  geom_45(color = "blue") +
  geom_smooth(color = "red") +
  theme_minimal()
...
ggplot(comp_pop,
       aes(x = age_3to5_count.pop_dfss,
           y = age_3to5_count.pop_may2023)) +
  geom_point(alpha = 0.3) + 
  geom_45(color = "blue") +
  geom_smooth(color = "red") +
  theme_minimal()
...
```{r replace data}
pop_by_age <- pop_by_age_2
```

```{r copies data for 2023}
pop_2023 <- 
  pop_by_age %>% 
  filter(year == 2022) %>%
  mutate(year = 2023)
