...
ests_with_forecast_wide_2 <- pop_0017_2010_2023_tract %>%
  mutate(GEOID = TRACT20) %>%
  select(-TRACT20, -TRACTCE20, -cca, -cca_name, -race, -agegroup) %>%
  select(GEOID, age, everything()) %>%
  arrange(GEOID, age)

ests_with_forecast_wide_2 <- ests_with_forecast_wide_2 %>%
  group_by(age, GEOID) %>%
  summarise(est2010 = sum(est2010),
            est2011 = sum(est2011),
            est2012 = sum(est2012),
            est2013 = sum(est2013),
            est2014 = sum(est2014),
...
pop_by_age_2 <- ests_with_forecast_wide_2 %>% 
  mutate(GEOID = as.character(GEOID),
