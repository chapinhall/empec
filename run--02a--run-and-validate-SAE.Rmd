...
plot_sae_comp_by_tract(sae_results_incpov_ctrls_mix_05_out,   
                       base_cat = incpov_ref_value, 
...
```{r function to redistribute estimates}
...
sae_to_redis <- sae_results_incpov_ctrls_100_05_out
sae_age <- "05" # "612"
residual_cat <- "400%+"
stopifnot(residual_cat %in% sae_to_redis$vc_value)
...
z <- 
  ...
  merge(pop_long %>% filter(age == pop_age, year == base_year),
        by = "GEOID",
        all.x = TRUE) %>% 
  # merge(acs5_incpov_cat_var %>% 
  #         filter(age == sae_age) %>% 
  #         select(PUMA, vc_value, acs5_tract.in.puma_sd),
  #       by = c("PUMA", "vc_value"),
  #       all.x = TRUE) %>% 
  # calculate variance %>% 
  ...
  mutate(
    ...
    # Rebuild distributed scales
    # share_model_infl_pct =       (infl_model_pct * share_model_trunc_m0),
    # share_model_infl_log = plogis(infl_model_log * share_model_trunc_m0),
    # share_fh_infl_pct    =       (infl_fh_pct    * share_fh_trunc_m0),
    # share_fh_infl_log    = plogis(infl_fh_log    * share_fh_trunc_m0),
    # share_model_redis_pct = share_model_infl_pct + acs1_5pct_pct,
    # share_model_redis_log = share_model_infl_log + acs1_5pct_pct,
    # share_fh_redis_pct    = share_fh_infl_pct    + acs1_5pct_pct,
    # share_fh_redis_log    = share_fh_infl_log    + acs1_5pct_pct
...
overs %>% 
  filter(sums>1.5) %>% 
  inner_join(z, by = "GEOID") %>% 
  select(GEOID, vc_value, share_model_trunc, share_model_redis_pct, share_model_redis_pct01) %>% 
  filter(vc_value == "400%+") %>% 
  pull(share_model_trunc)
```


```{r examine sums omitting resid cat}
omit_sums <- 
  z %>% 
  group_by(GEOID, PUMA) %>% 
  filter(vc_value != residual_cat) %>% 
  summarize(sums = sum(share_model_redis_pct01))

ggplot(omit_sums %>% filter(!is.na(sums)),
       aes(x = sums)) +
  geom_density()
```
...
check_redis_sae %>% 
  filter(!is.na(share_acs5),
         !is.na(share_model_trunc)) %>% 
  ggplot(aes(x = share_acs5,
             y = share_model_trunc)) +
  geom_45() + 
  geom_point(alpha = 0.1) +
...
check_redis_sae %>% 
  filter(!is.na(share_acs5),
         !is.na(share_model_redis_pct01_infl)) %>% 
  ggplot(aes(x = share_acs5,
             y = share_model_redis_pct01_infl)) +
  geom_45() + 
  geom_point(alpha = 0.1) +
...
est_vs_acs1 <- 
  ...
  select(GEOID, PUMA, vc_value, pop, share_acs1 = acs1_5pct_pct, 
         share_acs5, share_model_trunc, share_model_redis_pct, share_model_redis_pct01_infl) %>% 
