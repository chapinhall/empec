...
make_acs5_equiv <- function(sae_ests, sae_age, verbose = FALSE) {
  ...
  for (vc in vc_vals) {
    ...
    # value, i.e. kids in families based on both income and work eligibilty status.
...
if (developer_mode) {
  ...
  sae_results_ccdf_test %>% 
    group_by(vc_value) %>% 
    mutate(label = glue("{vc_value}\nrho = {round(cor(share_model_trunc, acs5_est, use = 'pairwise'), 2)}\nratio of ",
                        "var = {round(varNA(acs5_est)/varNA(share_model_trunc), 2)}")) %>% 
...
if (developer_mode) {
  ...
  sae_results_ccdf_test %>% 
    ...
    summarize(acs5_ccdf = sum(acs5_est),
...
redis_sae <- function(sae_to_redis, sae_age) {
  ...
  redis_out <- 
    ...
    merge(acs1_puma_formerge %>% 
            filter(age == acs_age) %>% 
            select(PUMA, vc_value, acs1_puma_pct),
          by = c("PUMA", "vc_value"),
          all.x = TRUE) %>% 
    ...
    mutate(
      ...
      acs1_puma_pct_log = qlogis(acs1_puma_pct),
      ...
      infl_acs1_acs5     = acs1_puma_pct / meanNA(share_acs5),
      infl_acs1_acs5_wgt = acs1_puma_pct / weighted.mean(share_acs5, w = pop, na.rm = TRUE),
      ...
      share_model_redis_pct =       (infl_model_pct * share_model_trunc_m0) + acs1_puma_pct,
      share_fh_redis_pct    =       (infl_fh_pct    * share_fh_trunc_m0)    + acs1_puma_pct,
      share_model_redis_log = plogis(infl_model_log * share_model_trunc_m0  + acs1_puma_pct_log),
      share_fh_redis_log    = plogis(infl_fh_log    * share_fh_trunc_m0     + acs1_puma_pct_log),
      share_acs5_redis_pct  =                         share_acs5_m0         + acs1_puma_pct,
...
est_vs_acs1 <- 
  check_redis_sae %>% 
  ungroup() %>%
  select(GEOID, PUMA, vc_value, pop, share_acs1 = acs1_puma_pct, 
