...
# /!\ Consider moving this to run--01d
acs5_incpov_cat <- 
  acs5tract %>% 
  select(matches("GEOID|incpov_(lt6|6to11).+_est")) %>% 
  pivot_longer(cols = -GEOID) %>% 
  mutate(age   = str_replace(name, "incpov_(.+)_r.+", "\\1"),
         range = str_replace(name, ".+\\d_(r.+)_est", "\\1")) %>% 
  select(-name) %>% 
  pivot_wider(names_from = "range",
              values_from = "value") %>% 
  mutate(r50to100 = r0to50 + r50to100) %>% 
  pivot_longer(cols = -c(GEOID, age)) %>% 
  # Put values into format that will merge into SAE `vc_value` field names
  mutate(vc_value = 
           name %>% 
           str_replace("r(\\d+)to(\\d+)", "\\1%-\\2%") %>% 
           str_replace("r(\\d+)plus", "\\1%+") %>% 
           str_replace("199", "200") %>% 
           str_replace("299", "300") %>% 
           str_replace("399", "400"))
...
acs5_incpov_cat_var <- 
  ...
  merge(geo_crosswalk %>% select(GEOID, PUMA) %>% unique(),
        by = "GEOID",
        all.x = TRUE) %>% 
  group_by(PUMA, age, vc_value) %>% 
  # /!\ Consider whether to build this as a population-weighted variance
  summarize(acs5_tract.in.puma_sd = sd(value, na.rm = TRUE)) %>% 
  ungroup()
```

```{r build tract-level estimates of ccdf eligibility using acs5 data}
# Our approach uses multiple ACS 5-year tables with partial information: income-
# to-poverty by age, and work status by family type for families with children.

# Because we do not know the joint distribution of these income- and work-eligibility
# phenomenal, we use microdata for some calibration. We use the phi calculation of
...
# Calculate correlations between income and work status across PUMAs
inc_work_elig_phi <- 
    acs1_child %>% 
    .[j = .(phi = cor(fam_incpov_ratio < local_ccdf_incratio_base/100,
                      ccdf_elig_incratio_225)),
      by = PUMA]

if (developer_mode) {
  print("Phi calculations -- correlation of income and work eligibility by PUMA")
  inc_work_elig_phi
}
...
# value, i.e. kids in families who are both income and work eligible. We
# can use calculations from the phi coefficient (https://en.wikipedia.org/wiki/Phi_coefficient)
...
#   phi = (n*n11 - n1.*n.1) / sqrt(n1.*n.1*(n - n1.)*(n - n.1))
...
# work eligibility--and where we have phi calculated. Note that phi=0 
# corresponds to the case of zero correlation, and would result in n11 = pq
# which is the joint probability case where both random variables are 
# independent, which confirms common expectations.

# Combine PUMA-level calculations of phi from the ACS 1-year data with
# the tract-level ACS 5-year calculations of marginal probabilities
load(file = glue("{output_path}acs5_ccdf_{my_output_tag}.Rda"))
ccdf_est_acs5 <- 
  acs5_ccdf %>% 
  select(GEOID, rIncPovLt200, wIncPovLt200, rWorkEligFam) %>% 
  merge(geo_crosswalk %>% select(GEOID, PUMA) %>% unique(),
        by = "GEOID",
        all.x = TRUE) %>% 
  merge(inc_work_elig_phi,
        by = "PUMA", 
        all.x = TRUE) %>% 
  mutate(
    ccdf_elig_acs5 = 
      phi * 
      sqrt(rIncPovLt200*rWorkEligFam*(1-rIncPovLt200)*(1-rWorkEligFam)) +
      rIncPovLt200*rWorkEligFam,
    ccdf_elig_acs5_indep = rIncPovLt200*rWorkEligFam)

if (developer_mode) {
  # Examine the distribution of values
  ggplot(ccdf_est_acs5,
         aes(x = ccdf_elig_acs5)) +
    geom_density()
  
  # Check that phi=0 corresponds to no difference, and neg/pos values 
  # affect calculations in anticipated ways
  ggplot(ccdf_est_acs5,
         aes(x = phi,
             y = ccdf_elig_acs5 - ccdf_elig_acs5_indep)) +
    geom_point(alpha = 0.2) +
    geom_hline(yintercept = 0) +
    geom_vline(xintercept = 0)
}

# Compare PUMA-aggregated calculations of ACS 5-year estimates with ACS 1-year
# estimates
if (developer_mode) {
  ccdf_est_puma_comp <- 
    ccdf_est_acs5 %>% 
    summarize(.by = PUMA,
              ccdf_elig_acs5 = weighted.mean(ccdf_elig_acs5, wIncPovLt200)) %>% 
    merge(acs1_child[between(AGEP, 0, 6),
                     j = .(ccdf_elig_acs1 = sum(PWGTP[ccdf_elig_incratio_225==TRUE])/sum(PWGTP)),
                     by = PUMA],
          by = "PUMA") 
  
  ccdf_est_puma_comp %>% 
    ggplot(aes(x = ccdf_elig_acs1,
               y = ccdf_elig_acs5)) +
    geom_point() + 
    geom_abline() +
  ...
  ccdf_est_puma_comp %>% 
    summarize(varNA(ccdf_elig_acs5), varNA(ccdf_elig_acs1))
}

# Compare tract-level estimates from ACS5 with tract-level SAE estimates
# Want to ideally see ~something~ not too far from the 45, and likely higher
# variance than the SAE
if (developer_mode) {
  comp_ccdf_acs5_sae <- 
    sae_results_ccdf_wk_pov_05$sae_out %>% 
    select(GEOID, share_model_trunc, vc_value) %>% 
    filter(vc_value %in% c("WorkElig_0%-100%",
                           "WorkElig_100%-225%")) %>% 
    summarize(ccdf_elig_sae = sum(share_model_trunc),
              .by = GEOID) %>% 
    merge(ccdf_est_acs5,
          by = "GEOID")
  
  comp_ccdf_acs5_sae %>% 
    summarize(varNA(ccdf_elig_sae), varNA(ccdf_elig_acs5))
  
  ggplot(comp_ccdf_acs5_sae,
         aes(x = ccdf_elig_sae,
             y = ccdf_elig_acs5)) +
    geom_abline() + 
    geom_point(alpha = 0.2)
...
```{r prep acs1_5pct data for merge}
acs1_5pct_puma_formerge <- 
  acs1_5pct_puma %>% 
  pivot_longer(cols = -c(GEOID, PUMA, age)) %>% 
  filter(!str_detect(name, "_n$")) %>% 
  mutate(vc_value =
           name %>% 
           str_replace(".+_(\\d+)_(\\d+).+", "\\1%-\\2%") %>% 
           str_replace(".+agg_(\\d+)_pct", "\\1%+")) %>% 
  rename(acs1_5pct_pct = value)
```
...
if (FALSE) {
  sae_to_redis <- sae_results_incpov_ctrls_100_05
  sae_age <- "05" # "612"
}
...
redis_sae <- function(sae_to_redis, sae_age) {
  ...
  redis_out <- 
    sae_to_redis$sae_out %>% 
    merge(acs1_5pct_puma_formerge %>% 
            filter(age == acs_age) %>% 
            select(PUMA, vc_value, acs1_5pct_pct),
          by = c("PUMA", "vc_value"),
          all.x = TRUE) %>% 
    merge(acs5_incpov_cat %>%
            filter(age == acs_age) %>%
            select(GEOID, vc_value, share_acs5 = value),
          by = c("GEOID", "vc_value"),
          all.x = TRUE) %>%
    ...
    mutate(
      ...
      acs1_5pct_pct_log = qlogis(acs1_5pct_pct),
      
      # Build variance inflation factors
      ...
      # Build a multiplicative inflation factor
      infl_acs1_acs5     = acs1_5pct_pct / meanNA(share_acs5),
      infl_acs1_acs5_wgt = acs1_5pct_pct / weighted.mean(share_acs5, w = pop, na.rm = TRUE),
        # We build this only for the acs5 shares because, under the current pilot
        # test stage, we are not planning to do a variance adjustment that the
        # SAE estimates would need
      
      # Redistribute using variance and mean adjustment
      share_model_redis_pct =       (infl_model_pct * share_model_trunc_m0) + acs1_5pct_pct,
      share_fh_redis_pct    =       (infl_fh_pct    * share_fh_trunc_m0)    + acs1_5pct_pct,
      share_model_redis_log = plogis(infl_model_log * share_model_trunc_m0  + acs1_5pct_pct_log),
      share_fh_redis_log    = plogis(infl_fh_log    * share_fh_trunc_m0     + acs1_5pct_pct_log),
      share_acs5_redis_pct  =                         share_acs5_m0         + acs1_5pct_pct,
        # We allow for an acs5-based estimate which simply adjusts means. The variance
        # of course doesn't have to be inflated, because it's already the benchmark
        # for inflation!
      
      # Redistribute using multiplication
      share_acs5_redis_mult      = share_acs5 * infl_acs1_acs5,
      share_acs5_redis_mult_wgt  = share_acs5 * infl_acs1_acs5_wgt,
      ...
      share_acs5_redis_pct01  = bound(share_acs5_redis_pct),
      
      share_acs5_redis_mult01      = bound(share_acs5_redis_mult),
      share_acs5_redis_mult_wgt01  = bound(share_acs5_redis_mult_wgt),
    ) %>% 
    group_by(GEOID) %>% 
    mutate(
      ...
      share_acs5_redis_pct01_infl  = share_acs5_redis_pct01  / sum(share_acs5_redis_pct01),
      
      share_acs5_redis_mult01_infl     = share_acs5_redis_mult01     / sum(share_acs5_redis_mult01),
      share_acs5_redis_mult_wgt01_infl = share_acs5_redis_mult_wgt01 / sum(share_acs5_redis_mult_wgt01),
    ) %>% 
    as.data.table()
  
  # Output the results to the `sae_out` slot of the input SAE
  sae_to_redis[["sae_out"]] <- redis_out
...
# Check on success
if (FALSE) {
  str_subset(cn(sae_results_incpov_ctrls_100_05$sae_out), "_infl$")
  sae_results_incpov_ctrls_100_05$sae_out %>% 
    select(GEOID, vc_value, 
           share_model_trunc, share_model_redis_pct, share_model_redis_pct01_infl)
  
  str_subset(cn(sae_results_ccdf_wk_pov_05$sae_out), "_infl$")
  sae_results_ccdf_wk_pov_05$sae_out %>% 
    select(GEOID, vc_value, 
           share_model_trunc, share_model_redis_pct, share_model_redis_pct01_infl)
}
...
check_redis_sae <- sae_results_incpov_ctrls_100_05$sae_out
```

```{r check the comparison of population-weighted vs unweighted acs5 inflation factors}
check_redis_sae %>% 
  ggplot(aes(x = infl_acs1_acs5, 
             y = infl_acs1_acs5_wgt)) + 
  geom_45() + 
  geom_point() + 
  facet_wrap(~vc_value)
```

```{r compare acs5 redis vs mult methods}
check_redis_sae %>% 
  ggplot(aes(x = share_acs5_redis_pct01_infl, 
             y = share_acs5_redis_mult01_infl)) + 
  geom_45() + 
  geom_point(alpha = 0.1) + 
  facet_wrap(~vc_value)
```
...
comp_plot("share_acs5", "share_acs5_redis_pct01_infl")
comp_plot("share_acs5_redis_pct01_infl", "share_acs5_redis_pct01_infl")
...
check_redis_sae %>% 
  ...
  summarize(cor_acs5_sae  = cor(share_acs5, share_model_trunc,            use = "pairwise") %>% round(3),
            ...
            cor_acs5_acs5 = cor(share_acs5, share_acs5_redis_pct01_infl, use = "pairwise") %>% round(3),
            ...
            var_acs5_infl = varNA(share_acs5_redis_pct01_infl) %>% round(3)) %>% 
  ungroup()
...
est_vs_acs1 <- 
  check_redis_sae %>% 
  ungroup() %>%
  select(GEOID, PUMA, vc_value, pop, share_acs1 = acs1_5pct_pct, 
         share_acs5, share_model_trunc, share_model_redis_pct,
         share_model_redis_pct01_infl, share_acs5_redis_pct01_infl) %>% 
  pivot_longer(cols = -c(GEOID, PUMA, vc_value, pop, share_acs1),
               names_to = "method",
               values_to = "estimate") %>% 
  group_by(PUMA, vc_value, method) %>% 
  summarize(
    # Checking: this should be unique
    share_acs1 = unique(share_acs1),
...
ggplot(est_vs_acs1,
  ...
  # The original SAE estimates (green) show clear compression
  # The raw redistributed estimates (blue) are highly consistent for each band
  # The truncated/reinflated estimates (purple) are also highly consistent for
