...
estimate_transition_counts <- function(sae_spec_name, share_calc_var, outcome_var, transition_data, age_prefix) {
  ...
  for (vc_val in vc_vals) {
    ...
    vIncpov[vIncpov == "ccdf"] <- as.character(local_ccdf_thresh)
    ...
    transition_data <- 
      ...
      .[j = c(vc_val) := 
          ...
          case_when(is.na(cWork)              ~ 1,
                    str_detect(cWork, "Not")  ~ 1 - ccdf_elig_loose,
                    TRUE                      ~     ccdf_elig_loose) *
  ...
  if (FALSE) {
    ...
    # <run the above>
    transition_data[j = .N, by = .(fn = share_WorkElig_100_ccdf, hand = 1*(ccdf_elig_loose)*between(fam_incpov_ratio_b, 1, local_ccdf_thresh/100))] %>% unique()
  ...
  for (cps_geo_subsample in c("none", "metros")) {
    ...
    for (sae_spec in specs) {
      ...
      if (str_detect(sae_spec, "^incpov")) {
        ...
        # The new strategy of using ACS1 means and ACS5 variance to redistribute
        # the sae calculations is only available so far for income-to-poverty
        # calculations
        # share_calc_var_use <- 
        #   ifelse(use_only_sae_model_estimates,
        #          "share_model_redis_pct01_infl",
        #          "share_fh_redis_pct01_infl")
        #share_calc_var_use <- "share_acs5_redis_pct01_infl"
        share_calc_var_use <- "share_acs5_redis_mult_wgt01_infl"
