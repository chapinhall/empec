...
for (ccdf_spec in str_subset(nowcast_parallel_setup$outcome_var, "ccdf_elig_incratio")) {
  sensitivity_to_add <- 
    nowcast_parallel_setup %>% 
  ...
  bind_list <- append(bind_list, sensitivity_to_add)
}
...
comp_nowcast_vs_sae(share_ests = share_pre_post %>% filter(str_detect(outcome_var, local_ccdf_incratio_base)),
                    title = case_when(my_output_tag == "IL" ~ "Tract-Level Now-Cast Estimates are Generally Above Baseline",
                                      TRUE ~ ""),
                    xlab = glue("Baseline (SAE) - {base_year}"),
                    ylab = glue("Now-Cast - {base_year+1}"),
