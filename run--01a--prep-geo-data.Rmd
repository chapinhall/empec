...
grffile <- glue("{input_path}grf23_lea_tract.xlsx")
if (!file.exists(grffile)) {
  download.file(url      = glue("https://nces.ed.gov/programs/edge/data/GRF23.zip"),
            destfile = glue("{input_path}GRF23.zip"))
  unzip(glue("{input_path}GRF23.zip"),
        files = "grf23_lea_tract.xlsx",
        exdir = str_replace(input_path, "\\/$", ""))
}

geo_crosswalk_sd <-
  readxl::read_xlsx(grffile) %>% 
  rename(school = NAME_LEA23,
         GEOID  = TRACT) %>% 
...
if (my_state_abbr == "IL") {
  b5_sd_file <- glue("{input_path}BirthToFive_SD2023_lookup.xlsx")
  b5_sd <-
