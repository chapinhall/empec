...
ggplot() +
  geom_sf(data = tractShp %>% filter(COUNTYFIP == "005"), #005 is Cumberland county, where Portland (largest population) is located.
          color = "red",   
          fill = NA) +
  geom_sf(data = sdShp %>% filter(GEOID %in% c(2300009, 2303780, 2303930, 2305670, 2306060, 2309930, 2310530, 2310710, 
...
ggplot() +
  geom_sf(data = tractShp %>% filter(COUNTYFIP == "005"),
          color = "red",   
          fill = NA) +
  geom_sf(data = sdShp %>% filter(GEOID == 2314837),
          color = NA, 
          fill = "green",
          alpha = 0.2) + 
  theme_void()
...
ggplot() +
  geom_sf(data = tractShp %>% filter(GEOID == "23003950100"),
          color = "red",   
          fill = NA) +
  geom_sf(data = sdShp %>% filter(school == "Caswell"),
          color = NA, 
          fill = "green",
          alpha = 0.2) + 
  geom_sf(data = sdShp %>% filter(school == "New Sweden"),
          color = NA, 
          fill = "yellow",
          alpha = 0.2) + 
  geom_sf(data = sdShp %>% filter(school == "School Administrative District 24"),
          color = NA, 
          fill = "yellowgreen",
          alpha = 0.2) + 
  geom_sf(data = sdShp %>% filter(school == "Westmanland"),
          color = NA, 
          fill = "blue",
          alpha = 0.2) + 
  geom_sf(data = sdShp %>% filter(school == "Regional School Unit 39"),
          color = NA, 
          fill = "purple",
          alpha = 0.2) + 
  geom_sf(data = sdShp %>% filter(school == "Aroostook Unorganized Territory"),
          color = NA, 
          fill = "gray",
          alpha = 0.2) + 
  theme_void()
...
ggplot() +
  geom_sf(data = sdShp %>% filter(school == "Auburn"), 
          color = "blue",   
          fill = "blue") +
  geom_sf(data = sdShp %>% filter(school == "Bangor"), 
          color = "green",   
          fill = "green") +
  geom_sf(data = sdShp %>% filter(school == "Lewiston"), 
          color = "yellow",   
          fill = "yellow") +
  geom_sf(data = sdShp %>% filter(school == "Portland"), 
          color = "red",   
          fill = "red") +
  geom_sf(data = sdShp %>% filter(school == "Sanford"), 
          color = "purple",   
          fill = "purple") +
  geom_sf(data = sdShp,
          color = "black",   
...
if (FALSE) {
  ggplot() +
    geom_sf(data = tractShp %>% filter(COUNTYFIP %in% c("001")),
            color = "red",   
            fill = NA) +
    geom_sf(data = sdShp %>% filter(school %in% c("Auburn")),
            color = NA, 
            fill = "green",
            alpha = 0.2) + 
    theme_void()
  ...
  ggplot() +
    geom_sf(data = tractShp %>% filter(COUNTYFIP %in% c("019")),
            color = "red",   
            fill = NA) +
    geom_sf(data = sdShp %>% filter(school %in% c("Bangor")),
            color = NA, 
            fill = "green",
            alpha = 0.2) + 
    theme_void()
  ...
  ggplot() +
    geom_sf(data = tractShp %>% filter(COUNTYFIP %in% c("001")),
            color = "red",   
            fill = NA) +
    geom_sf(data = sdShp %>% filter(school %in% c("Lewiston")),
            color = NA, 
            fill = "green",
            alpha = 0.2) + 
    theme_void()
  ...
  ggplot() +
    geom_sf(data = tractShp %>% filter(COUNTYFIP %in% c("005")),
            color = "red",   
            fill = NA) +
    geom_sf(data = sdShp %>% filter(school %in% c("Portland")),
            color = NA, 
            fill = "green",
            alpha = 0.2) + 
    theme_void()
  ...
  ggplot() +
    geom_sf(data = tractShp %>% filter(COUNTYFIP %in% c("031")),
            color = "red",   
            fill = NA) +
    geom_sf(data = sdShp %>% filter(school %in% c("Sanford")),
            color = NA, 
            fill = "green",
            alpha = 0.2) + 
    theme_void()
...
# sdShp downloaded from the Census does not contain tract information
...
if (FALSE) {
  ...
  ggplot() +
    ...
    geom_sf(data = sdShp %>% filter(school %in% c("Portland")),
            ...
            fill = "green",
            alpha = 0.2) + 
    geom_sf(data = sdShp %>% filter(school %in% c("Long Island")),
            color = NA, 
            fill = "blue",
            alpha = 0.2) + 
    theme_void()
...
geo_objects <- c("my_state_fip", "geo_crosswalk", "geo_crosswalk_zcta", "geo_crosswalk_sd", "tractShp", "pumaShp", "countyShp", "zctaShp", "leaflet_base_map")
