...
geo_crosswalk_zcta <-
  ...
  .[grepl("^(60|61|62)", GEOID_ZCTA5_20)] %>%
  .[grep("^17", GEOID_TRACT_20)] %>%
  ...
  # Note that distinct numbers of GEOID changed from 2010
  # Issue with no-TRPOPPCT-in-2020 (The Percentage of Total Population of the Census Tract represented by the record)
  # 2010: https://www.census.gov/programs-surveys/geography/technical-documentation/records-layout/2010-zcta-record-layout.html
  # 2020: https://www.census.gov/programs-surveys/geography/technical-documentation/records-layout/2020-zcta-record-layout.html
...
pumaShp   <-    pumas(state = my_state_fip, cb = TRUE, year = tiger_year)                           #/!\Error: Cartographic boundary PUMAs are not yet available for years after 2019. Use the argument `year = 2019` instead to request your data.
countyShp <- counties(state = my_state_fip, cb = TRUE, year = tiger_year)
zctaShp   <-    zctas(cb = TRUE, year = tiger_year, starts_with = zcta_starts)                      #/!\Error: The Census Bureau has not yet released the CB ZCTA file for 2022. Please use the argument `year = 2020` or `cb = FALSE` instead.
...
```{r gets puma and zcta shapefile from Census website}
for (geo in c("puma20", "zcta520")) {
  census_suffix <- switch(geo, "puma" = "puma20", "zcta" = "zcta520")
  census_path   <- switch(geo, 
                          "puma"   = glue("https://www2.census.gov/geo/tiger/TIGER{tiger_year}/PUMA/"),
                          "zcta"   = glue("https://www2.census.gov/geo/tiger/TIGER{tiger_year}/ZCTA520/"))
  if (geo == "puma20") {
    download_filename <- glue("state_{my_state_fip}_{census_suffix}.zip")
    download.file(url      = glue("{census_path}tl_{tiger_year}_{my_state_fip}_{census_suffix}.zip"),
    ...
    download_filename <- glue("us_{census_suffix}")
    download.file(url      = glue("{census_path}tl_{tiger_year}_us_{census_suffix}.zip"),
                  destfile = glue("{input_path}{download_filename}.zip"))
    unzip(glue("{input_path}tl_{tiger_year}_us_{geo}.zip"),
          exdir = str_replace(input_path, "\\/$", ""))
    }
}  
