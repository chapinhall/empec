...
sumup_moment_dt <- function(theta, x) {
  # Extract the individual x (long by g+t) component of data
  x$ix_gt %>% 
...
sumup_grad_dt <- function(x) {
  x$ix_gt %>% 
    .[j = lapply(1:max(time), function(t) 1*(time == t))] %>% 
    t()
}
...
avgx_grad_dt <- function(x) {
  ...
  x$ix_gt %>% 
    ...
    .[j = lapply(1:nrow(x$thetas_avgx),
                 function(row_of_avg) {
                   
                   s <- x$params[["s"]]
                   my_avg_row <- x$thetas_avgx[row_of_avg]
...
trans_moment_dt <- function(theta, x) {
  x$ix_gt %>% 
    ...
    .[j = theta0_lag := shift(theta0, n = 1, type = "lag"),
      by = g] %>% 
    .[!is.na(theta0_lag)] %>% 
    pull(theta0 - x$params[["beta"]]*theta0_lag)
}
...
```{r}
make_model <- function(my_theta0, my_x) {
...
```{r }
toy_model <- make_model(my_theta0 = 0, my_x = toy)
