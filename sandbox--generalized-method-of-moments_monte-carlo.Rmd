...
draw_obs_data <- 
  function(se_est,          # Standard error of each theta given sample variation
    ...
    thetas_avgx <- make_avgx(thetas_avgx, span_length)
...
avgx_moment_dt <- function(theta, x, s) {
...
# lapply().
avgx_grad_dt <- function(x, s) {
  
  span_structure <- 
    expand.grid(time = s:max(x$time), 
                g = 1:max(x$g))
  # Start with the long structure for the gxt's
  x %>% 
    ...
    .[j = lapply(1:nrow(span_structure),
                 function(row_of_str) {
                   
                   my_avg_row <- span_structure[row_of_str,]
...
```{r}
trans_moment_dt <- function(theta, x, beta) {
  x %>% 
    .[j = theta0 := theta] %>% 
...
trans_grad_dt <- function(x) {
  A <- max(x$time)
  ix_wide <- x[time != 1] %>% select(g, time) %>% t()
  x %>% 
...
my_s    <- my_setup$params[["s"]]
my_x    <- my_setup$ix_gt
my_beta <- my_setup$params[["beta"]]
...
system.time({
  out <- 
    optim(par = rep(-5, length(theta0)), #my_setup$thetas_gt$theta_gt, # theta0,
          fn = obj,
          gr = obj_grad,
          method = "BFGS")
})
