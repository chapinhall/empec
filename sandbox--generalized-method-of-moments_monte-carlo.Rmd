...
Let $\theta_{gt}$ represent a given characteristic describing a population local to geography $g$ at time $t$. A motivating example is the number of children in families that qualify for Head Start in a given Census tract, for a given year.

Given available Census data, we establish a series of conditions that these $\theta_{gt}$s should meet, and seek to estimate them in a Generalized Method of Moments framework. 
    ...
    thetas_avgx <- 
      bind_rows(thetas_avgx,
                dt[time %in% (i - span_length + 1):i,
                   .(time   = i,
                     x_gs   = mean(x_gt),
                     se_avg = sqrt(mean(se_est^2)/span_length)),
                   by = g])
  }
  thetas_avgx
}
...
draw_obs_data <- 
  function(se_est      = 0,    # Standard error of each theta given sample variation
...
Confirming the above, we have $q = \underbrace{T}_\text{sum up} + \overbrace{G(T-s+1)}^\text{average across} + \underbrace{G(T-1)}_\text{transition}$ moment conditions, and $k = G \times T$ parameters.
...
Each moment condition thus takes the form of $\{\frac{1}{s}\Sigma_{i=t-s+1}^{t}\theta_{gi} - \bar{x}_{gt} = 0\}_{gt}$. We have $G \times (T-s+1)$ of these moment conditions.
...
trans_moment_dt <- function(theta, x) {
  ...
  x$ix_gt %>% 
    ...
    .[j = theta0_lag := shift(theta0, n = 1, type = "lag"),
      by = g] %>% 
    .[!is.na(theta0_lag)] %>% 
    mutate(diff = theta0 - beta*theta0_lag) %>% 
    pull(diff)
}
...
collect_moments <- 
  function(theta, x) {
      c("sumup" = sumup_moment_dt(theta = theta, x = x),
        "avg"   =  avgx_moment_dt(theta = theta, x = x),
        "trans" = trans_moment_dt(theta = theta, x = x))
  }

# Check that moments are 0 for true
if (FALSE) {
  collect_moments(full_noerr$ix_gt$x_gt, full_noerr) %>% 
    round(4) %>% 
...
```{r function to implement estimation for a given problem}

implement_gmm <- function(my_setup, use_weights = FALSE) {
  ...
  if (use_weights) {
    mom_se <- 
      c(my_setup$thetas_sumup$se_sum,
        my_setup$thetas_avgx$se_avg,
        rep(my_setup$params[["sd_trans"]], 
            nrow(my_setup$ix_gt[time > min(time)])))
    mom_wgt <- 1/(mom_se^2)
    mom_wgt[is.nan(mom_wgt) | is.infinite(mom_wgt)] <- 1  
  } else {
    mom_wgt <- 1
  }
  
  # /!\ add weights for precision
  obj <- function(theta0) {
    m <- moms(theta0)
    (t(m)*mom_wgt) %*% m
  }
...
```{r run different specs}

out_toy        <- implement_gmm(toy)
out_full_noerr <- implement_gmm(full_noerr)
out_full       <- implement_gmm(full)
out_full_wgt   <- implement_gmm(full, use_weights = TRUE)

```


```{r check output}
cbind(estimated_pars = out_full_noerr$est$par, 
      full_noerr$ix_gt) %>% 
  head()
...
full_noerr$ix_gt %>% 
  select(-theta0) %>% 
  mutate(est  = out_full_noerr$est$par,
         init = out_full_noerr$theta0) %>% 
  ggplot(aes(x = x_gt,
             y = est)) + 
...
```{r compare estimates across runs}
ggplot() +
  aes(x =     out_full$est$par,
      y = out_full_wgt$est$par) +
  geom_point() + 
  geom_abline() 

cor(    out_full$est$par,
    out_full_wgt$est$par)

est_comp <- 
  data.frame(true      = out_full$input$thetas_gt$theta_gt,
             est_unwgt = out_full$est$par,
             est_wgt   = out_full_wgt$est$par) %>% 
  mutate(err_unwgt = est_unwgt - true,
         err_wgt   = est_wgt   - true)

ggplot(est_comp) +
  aes(x = true) +
  geom_point(aes(y = est_unwgt),
             color = "darkgreen") +
  geom_point(aes(y = est_wgt),
             color = "orange") +
  geom_abline()
...
# Develop Real-World Data Input for GMM

```{r}
#| label: select a previous ELPEP run to work from

elpep_customization_file <- "C:/Users/mader-nick1/OneDrive - National Opinion Research Center/Documents/projects/elpep/il-ests/Interface for Specifying Custom ELPEP Eligibility Estimates - Chicago.xlsm"
source("settings--main.R")
source("method--read-customization-file.R")
source("method--pull-and-process-acs5.R")

output_path <- "C:/Users/mader-nick1/OneDrive - National Opinion Research Center/Documents/projects/elpep/Illinois ests from CH/"
...
```{r}
#| label: pull income distribution

try(detach("package:sae", unload = TRUE))
try(detach("package:emdi", unload = TRUE))
try(detach("package:lme4", unload = TRUE))
try(detach("package:MASS", unload = TRUE))
try(detach("package:Hmisc", unload = TRUE))
lPull_acs5 <- 
  lapply(c("B19080", "B19081", "B19082"), 
         pull_table,
         year = 2023,
         state = "IL",
         county = NULL)
lReshaped_acs5 <- 
  lapply(lPull_acs5,
         \(x) {
           x %>% 
             select(GEOID, quint_meas, quintile, est = estimate, se) %>% 
             pivot_longer(cols = c(est, se)) %>% 
             unite(label, quint_meas, quintile, name) %>% 
             pivot_wider(names_from = label,
                         values_from = value)
           })

acs5_quintiles <- 
  Reduce(\(x,y) merge(x, y, by = "GEOID"), 
         lReshaped_acs5)
...
```{r}
#| label: combine ACS5 data

acs5tract_gmm <- 
  acs5tract %>% 
  select(GEOID, incpov_r0to100_est, incpov_r0to100_se) %>% 
  merge(acs5_quintiles %>% select(matches("GEOID|upper")),
        by = "GEOID")

```

```{r}
#| label: examine and cultivate CPS

tabyl(cps_child_base$YEAR)
  # This shows that we have 2019:2023, which is consistent with the new ACS data pulls (at least)

cps_gmm <- 
  cps_child_base[YEAR %in% 2019:2023] %>% 
  rename(fam_incpov_ratio_base = fam_incpov_ratio) %>% 
  merge(cps_post %>% 
          rename(fam_incpov_ratio_post = fam_incpov_ratio) %>% 
          select(-YEAR, -MONTH),
        by = c("CPSID"))
...
cps_gmm %>% 
  split(cps_gmm$YEAR) %>% 
  map(\(df) {
    lm(incpov_le100_post_adj ~ fam_incpov_ratio_cat_by100 == "0%-100%", 
     data = df)  
  }) %>% 
  map_dfr(\(reg) tidy(reg))
