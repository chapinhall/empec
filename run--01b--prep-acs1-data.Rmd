           ...
           case_when(
             str_detect(work_incpov_statushs,
                        ...
                        glue("NotWorkElig_(0%|100%)")) ~ 
                          glue("NotWorkElig_0%_{local_ccdf_thresh}%"),
             str_detect(work_incpov_statushs,
                        glue("NotWorkElig_[^(0%|100%)]")) ~
                          glue("NotWorkElig_{local_ccdf_thresh}_500%"),
...
elig_stats_puma <- 
  ...
  mutate(ccdf_inc_elig = fam_incpov_ratio_cat_hs_ccdf %in% 
                           c("0%-100%", glue("100%-{local_ccdf_thresh}%")),
...
(puma_agg <- 
   ...
   filter(between(AGEP, 0, 5)) %>% 
   group_by(PUMA) %>% 
   mutate(faminc_lt_thresh = fam_incpov_ratio <= local_ccdf_thresh/100,
...
acs1_tables <- 
  get_acs(geography = "puma", 
          state     = my_state_abbr,
          county    = ifelse(exists("my_county"), my_county, NULL),
          survey    = "acs1",
          table     = "B17024",
          year      = base_year)
...
# /!\ The API pulls of ACS1 aggregate tables is resulting in NAs. These code 
# chunks reflect ad hoc, manual pulls of individually pulled problematic PUMAs 
# to avoid these missings
if (base_year == 2022) {
  acs1_tables_labeled <-
    openxlsx::read.xlsx(glue("{input_path}acs_puma_cook_b17024_2022.xlsx"),
                        sheet = "in") %>%
    merge(acs1_tables_labeled %>% select(variable, label) %>% unique(),
          by = "variable",
          all.x = TRUE)
} else if (base_year == 2018) {
  acs1_tables_labeled <-
    read.csv(glue("{input_path}acs_pov_estimates_2018.csv"),
             stringsAsFactors = FALSE) %>%
    mutate(estimate = as.numeric(str_replace_all(estimate, ",", "")),
           moe      = as.numeric(str_replace_all(moe,      ",", ""))) %>% 
    merge(acs1_tables_labeled %>% select(variable, label) %>% unique(),
          by = "variable",
          all.x = TRUE)
}
