...
```{r examine puma-level correspondence between income eligibility and work eligibility, eval = developer_mode}
...
ggplot(workelig_and_incelig_by_puma,
  ...
  scale_y_continuous(labels = percent,
                     limits = c(0, NA)) +
  coord_fixed() +
  labs(title = glue("PUMA-level Work vs Income-Eligibility for {local_ccdf_name_short}, Children 0-5"),
       # /*\ Note: check that the pattern and thus interpretation holds
       subtitle = glue("The negative relationship leads to a stable overall {local_ccdf_name_short} eligiblity rate\n",
                       "as PUMAs with lower incomes and lower labor market participation can \n",
                       "end up with eligibility rates similar to PUMAs with relatively higher income\n",
