...
write_agg_output <- function(outcomes_regex, age_regex = ".+", agg_levels, out_label) {
  ...
  if (file.exists(excel_front_page_file)) {
  ...
  for (lvl in agg_levels) {
    add_sheet <-
      ...
      mutate(
        # Recode numeric age values
        Age = 
          age %>% 
          str_replace("age_(\\d+)to(\\d+)", "Age \\1-\\2") %>% 
          str_replace("age_(\\d+)$", "Age \\1"),
        
        # Recode non-numeric age values
        Age = 
          case_when(Age %like% "infant"  ~ "Infant (1-15 months)",
                    Age %like% "toddler" ~ "Toddler (16-36 months)",
                    Age == "Age 3" ~ "Pre-K 3 (37-48 months)",
                    Age == "Age 4" ~ "Pre-K 4 (49-60 months)",
                    TRUE             ~ Age),
