...
Current Population Survey (CPS) microdata are manually downloaded from the [IPUMS CPS](https://cps.ipums.org/cps/) site from `r base_year` through the most recent available month, and 
...
cps_pov <-
  ...
  # Bin family income
  .[j = `:=`(
             # Develop categories of household income to align with those in ACS1, which define the SAE targets
             fam_incpov_ratio_cat_byMix       = bin_incpov_ratio(fam_incpov_ratio,     pov_breaks = c(0, 0.50, 1.00, 2.00, 4.00)),
             fam_incpov_ratio_cat_byMix_adj   = bin_incpov_ratio(fam_incpov_ratio_adj, pov_breaks = c(0, 0.50, 1.00, 2.00, 4.00)),
             fam_incpov_ratio_cat_hs_ccdf     = bin_incpov_ratio(fam_incpov_ratio,     pov_breaks = c(0, 1.00, local_ccdf_thresh/100, 3.50, 5.00)),
             fam_incpov_ratio_cat_hs_ccdf_adj = bin_incpov_ratio(fam_incpov_ratio_adj, pov_breaks = c(0, 1.00, local_ccdf_thresh/100, 3.50, 5.00)),
             fam_incpov_ratio_cat_to185       = bin_incpov_ratio(fam_incpov_ratio,     pov_breaks = c(0, 0.50, 1.25, 1.85, 4.00)),
             fam_incpov_ratio_cat_to185_adj   = bin_incpov_ratio(fam_incpov_ratio_adj, pov_breaks = c(0, 0.50, 1.25, 1.85, 4.00)),
             ...
             fam_incpov_ratio_cat_by75_adj    = bin_incpov_ratio(fam_incpov_ratio_adj, pov_breaks = c(0, 0.75, 1.50, 2.25, 4.00)),
...
```{r if requested -- generate flags for income to custom threshold ratio}
if (exists("custom_income_thresh")) {
  # Check that all family sizes present in the data are represented in the custom
  # spec file
  if (!maxNA(cps_pov$fam_size) %in% custom_income_thresh$fam_size) {
    
    warning(glue("The `custom_income_thresh` table does not contain all family sizes present in the Current Population Survey microdata. It must reflect family sizes up through `maxNA(cps_pov$fam_size)`")) 
  }
  ...
  cps_pov <- 
    ...
    merge(custom_income_thresh,
          by = "fam_size",
          all.x = TRUE) %>% 
    .[j = incratio_thresh := faminc_adj / inc_thresh]
}
```
...
* have family income below [allowable limits](https://www.dhs.state.il.us/page.aspx?item=118832), representing income below 225% of the federal poverty line for initial eligibility, and below 275% of FPL for 12-month redetermination
...
```{r if requested -- build ccdf eligibility flags relative to custom thresholds}
if (exists("custom_income_thresh")) {
  ...
  if (!maxNA(cps_pov$fam_size) %in% custom_income_thresh$fam_size) {
    ...
    warning(glue("The `custom_income_thresh` table does not contain all family ",
                 "sizes present in the Current Population Survey microdata. It ",
                 "must reflect family sizes up through `maxNA(cps_pov$fam_size)`")) 
  }
...
cps_elig <- 
  ...
  .[j = paste0("ccdf_elig_incratio_", local_ccdf_incratio_cuts) :=
      lapply(local_ccdf_incratio_cuts, 
             function(x) {
               1*ccdf_elig_tight*(my_incratio <= x/100)
             })]
```
...
cps_elig[j = .(n = comma(.N)), 
         by = .(#fam_incpov_ratio_cat_by100,
                fam_incpov_ratio_cat_by100_adj,
                #ccdf_elig_tight_inc,
                get(glue("ccdf_elig_incratio_{local_ccdf_incratio_base}")))] %>% 
...
cps_child_base <- 
  ...
  .[j = c("CPSID", "CPSIDP", "WTFINL", "HWTFINL", 
          ...
          "fam_incpov_ratio_cat_hs_ccdf", "fam_incpov_ratio_cat_hs_ccdf_adj",
          str_subset(cn(cps_elig), "ccdf_elig_incratio_")),
    with = FALSE] %>% 
  .[j = `:=`(fSpouse_present = 
               factor(spouse_present,
                      levels = 0:1, 
...
cps_post <-
  cps_elig[is_post == TRUE & RELATE == 101 & months_since_base == 15,
           j = c("CPSID",
                 ...
                 "faminc_adj", "fam_incpov_ratio",
                 "fam_incpov_ratio_le.5",
                 "fam_incpov_ratio_le.5_adj",
                 "fam_incpov_ratio_le1",
                 "fam_incpov_ratio_le1_adj",
                 "fam_incpov_ratio_le1.85",
                 "fam_incpov_ratio_le1.85_adj",
                 "fam_incpov_ratio_le2",
                 "fam_incpov_ratio_le2_adj",
                 "fam_incpov_ratio_le2.25",
                 "fam_incpov_ratio_le2.25_adj",
                 "fam_incpov_ratio_le4.00",
                 "fam_incpov_ratio_le4.00_adj",
                 str_subset(cn(cps_elig), "ccdf_elig_incratio_")),
           with = FALSE] %>% 
  .[j = hoh_empl_post := 1*(EMPSTAT == 10)] %>% 
  rename(incpov_le50_post      = fam_incpov_ratio_le.5,
