...
# for (ccdf_spec in unique(str_subset(nowcast_elig_counts_final$outcome_var, "ccdf_elig_incratio_"))) {
  # my_out_label <- 
  #   str_replace(ccdf_spec,
  #               "ccdf_elig_incratio_(\\d+)",
  #               "{local_ccdf_name_short} - \\1%")
  
  write_agg_output(x = nowcast_agg_zip %>% filter(outcome_var %like% "ccdf_elig_incratio_"), 
                   agg_level = "ZCTA",   
                   sheet_name = "Zip Code", 
                   add = FALSE,
                   out_label = local_ccdf_name_short)
  
  write_agg_output(x = nowcast_agg_cty %>% filter(outcome_var %like% "ccdf_elig_incratio_"),
                   agg_level = "County", 
                   sheet_name = "County",
                   add = TRUE,
                   out_label = local_ccdf_name_short)
  ...
  if (exists("my_aux_geo")) {
    ...
    write_agg_output(x = nowcast_ests_aux %>% filter(outcome_var  %like% "ccdf_elig_incratio_"),
