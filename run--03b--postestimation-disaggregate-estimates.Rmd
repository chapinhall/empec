...
for (ccdf_spec in str_subset(nowcast_parallel_counts$outcome_var, "ccdf_elig_incratio")) {
...
for (ccdf_spec in str_subset(nowcast_elig_counts_final$outcome_vars, "ccdf_elig_incratio_")) {
  ...
  write_agg_output(x = nowcast_agg_zip %>% filter(outcome_var == ccdf_spec), 
                   agg_level = "ZCTA",   
                   sheet_name = "Zip Code", 
                   add = FALSE,
                   out_label = my_out_label)
  
  write_agg_output(x = nowcast_agg_cty %>% filter(outcome_var == ccdf_spec),
                   agg_level = "County", 
                   sheet_name = "County",
                   add = TRUE,
                   out_label = my_out_label)
  ...
  if (exists("my_aux_geo")) {
    ...
    write_agg_output(x = nowcast_ests_aux %>% filter(outcome_var == ccdf_spec),
