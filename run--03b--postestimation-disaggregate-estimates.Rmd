...
calc_breakdown <- function(acs1_outcome_sub, outcome_var) {
  acs1_age_breakdown <-
    acs1_outcome_sub %>% 
    filter(child_agegroup %in% c("0to2", "3to5") & YEAR == base_year) %>% 
...
age_breakdown_byoutcome <-
 bind_rows(calc_breakdown(acs1_child %>% filter(fam_incpov_ratio_cat_mix   == "0%-50%"),
           ...
           calc_breakdown(acs1_child %>% filter(fam_incpov_ratio_cat_by100 == "0%-100%"), 
                          "hs_elig_post_adj"),
           calc_breakdown(acs1_child %>% filter(fam_incpov_ratio_cat_mix2  %in% c("0%-100%", "100%-150%")), 
                          "le150_elig_post_adj"),
...
```{r develop and combine microdata from both acs1 and cps}
acs1_fpl_agecounts <- 
  acs1_child %>%
  .[child_agegroup %in% c("0to2", "3to5", "6to8", "9to12") & !is.na(fam_incpov_ratio_cat_mix2) ] %>%
  .[j = .(n_aiy = sum(PWGTP)), #age, income, year
    by = .(year = YEAR, age = child_agegroup, incpov = fam_incpov_ratio_cat_mix2)] %>%
  .[j = source := "acs1"]

cps_fpl_agecounts <- 
  cps_child_base %>%
  .[child_agegroup %in% c("0to2", "3to5", "6to8", "9to12") & !is.na(fam_incpov_ratio_cat_byMix2) & STATEFIP == my_state_fip] %>%
  .[j = .(n_aiy = sum(WTFINL)), #age, income, year
    by = .(year = YEAR, age = child_agegroup, incpov = fam_incpov_ratio_cat_byMix2)] %>%
  .[j = source := "cps"]

fpl_agecounts <- 
  bind_rows(acs1_fpl_agecounts, cps_fpl_agecounts) %>%
  .[j = n_ay := sum(n_aiy), #age, year
    by = .(year, age, source)]
```


```{r develop microdata from both acs1 and cps}
# /!\ Given the small size of the CPS when subset to the state level, especially
# with further disaggregation, consider using bootstrap facilities to examine
# the reliability of even these state-wide statistics
# See e.g.: https://www.statology.org/bootstrapping-in-r/
fpl_agecounts <- 
  bind_rows(fpl_agecounts,
            fpl_agecounts[age %in% c("0to2", "3to5"),
                          j = .(n_aiy = sum(n_aiy),
                                n_ay  = sum(unique(n_ay))),
                          by = .(year, incpov, source)] %>% 
              .[j = age := "0to5"],
            fpl_agecounts[age %in% c("0to2", "3to5", "6to8", "9to12"),
                          j = .(n_aiy = sum(n_aiy),
                                n_ay  = sum(unique(n_ay))),
                          by = .(year, incpov, source)] %>% 
              .[j = age := "0to12"])  %>%
  .[j = `:=`(pct = n_aiy / n_ay,
             age = factor(age,
                          levels = c("0to2", "3to5", "0to5", "6to8", "9to12", "0to12")))] %>% 
  arrange(year, age, incpov, source)

ggplot(fpl_agecounts[!str_detect(incpov, "225%\\+")], 
       aes(x = year,
           y = pct,
           color = source)) +
  scale_y_continuous(label = percent) +
  facet_grid(age~incpov) +
  geom_line() +
  geom_point() +
  theme_bw()

merge(fpl_agecounts[source == "acs1"] %>% dplyr::select(-source),
      fpl_agecounts[source == "cps"]  %>% dplyr::select(-source),
      by = c("year", "age", "incpov"),
      suffixes = c("_acs1", "_cps"),
      all = TRUE) %>%
  write.csv(file = glue("{output_path}ACS and CPS counts and percents -- 2019-2022, 0-100pct, 100-150pct, 150-225pct.csv"),
            row.names = FALSE)
...
nowcast_agg_top   <- agg_tracts_up(nowcast_elig_counts_final, "top")
...
```{r compare top-level estimates from microdata and elpep}
top_comp <- 
  nowcast_agg_top %>%
  mutate(year = base_year + ifelse(pd == "base", 0, 1)) %>%
  filter(age != "age_6to12") %>%
  pivot_wider(id_cols = c("year", "age"), 
              names_from = "outcome_var",
              values_from = "share") %>%
  mutate(`0%-100%`   = hs_elig_post_adj,
         `100%-150%` = le150_elig_post_adj - hs_elig_post_adj,
         `150%-225%` = upk_elig_post_adj - le150_elig_post_adj,
         `225%+`     = 1 - upk_elig_post_adj,
         age         = str_replace(age, "age_", "")) %>%
  dplyr::select(-ends_with("_adj")) %>%
  pivot_longer(cols      = -one_of(c("year", "age")),
               names_to  = "incpov",
               values_to = "pct") %>%
  # Base year calculations with the 150% cutpoint are invalid (since we don't get
  # that calculation in the SAE method
  # Remove any related calculations for the base year 
  filter(!(year == base_year & str_detect(incpov, "150%"))) %>% 
  mutate(source = "elpep") %>%
  bind_rows(fpl_agecounts)

ggplot(top_comp %>%
         filter(!str_detect(incpov, "225%\\+")), 
       aes(x = year,
           y = pct,
           color = source)) +
  scale_y_continuous(label = percent) +
