...
calc_breakdown <- function(acs1_outcome_sub, outcome_var) {
  acs1_age_breakdown <-
    ...
    mutate(child_agegroup = glue("age_{child_agegroup}")) %>% 
...
# /!\ Simplify the filtering conditions to use the incpov field
age_breakdown_byoutcome <-
 bind_rows(
   calc_breakdown(acs1_child %>% filter(fam_incpov_ratio_cat_mix   == "0%-50%"),
...
nowcast_agg_zip   <- agg_tracts_up(nowcast_elig_counts_final, "ZCTA") %>% 
  mutate(ZCTA = str_pad(ZCTA, side = "left", width = 5, pad = "0"))
...
outcomes_out_regex <- "ccdf_elig_incratio_|incpov_le(100|200)"

write_agg_output(x = nowcast_agg_zip %>% filter(outcome_var %like% outcomes_out_regex), 
                 agg_level = "ZCTA",   
                 sheet_name = "Zip Code", 
                 add = FALSE,
                 out_label = local_ccdf_name_short)

write_agg_output(x = nowcast_agg_cty %>% filter(outcome_var %like% outcomes_out_regex),
                 agg_level = "County", 
                 sheet_name = "County",
                 add = TRUE,
                 out_label = local_ccdf_name_short)
...
if (exists("my_aux_geo")) {
  ...
  write_agg_output(x = nowcast_ests_aux %>% filter(outcome_var  %like% outcomes_out_regex),
