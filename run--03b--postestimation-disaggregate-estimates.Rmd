...
geo_crosswalk <- 
  geo_crosswalk %>% 
  merge(geo_crosswalk_aux %>% select(GEOID, community, pct_area),
        by = "GEOID",
        all.x = TRUE) %>% 
  mutate(chicago_flag = ifelse(!is.na(community), 1, 0),
         County2 = case_when(County != "Cook" ~ County,
                             County == "Cook" & chicago_flag == 0 ~ "Cook (Non-Chicago)",
                             County == "Cook" & chicago_flag == 1 ~ "Cook (Chicago)"),
