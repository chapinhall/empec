...
age_breakdown_byoutcome <-
 bind_rows(
   ...
   calc_breakdown(acs1_child %>% filter(work_incpov_statushs %in% c("WorkElig_0%-100%", glue("WorkElig_100%-{local_ccdf_thresh}%"))),
...
if (exists("my_aux_geo")) {
  nowcast_agg_aux <- agg_tracts_up(nowcast_elig_counts_final, "aux_geo_label")  #/!\Warning: Detected an unexpected many-to-many relationship between `x` and `y`.
}
```
...
```{r write various levels of geographic aggregation out to file for le50, le100, le200}
if (exists("my_aux_geo")) {
