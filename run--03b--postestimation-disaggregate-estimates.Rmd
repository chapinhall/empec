...
write_agg_output(outcomes_regex = outcomes_out_regex,
                 age_regex      = "age_(0to2|3to5|4|0to5|6to12)",
...
static_map_val <- function(estimates,
  ...
  my_plot <-
    ...
    labs(title    = ifelse(!is.null(map_title), map_title, str_wrap(measure_label)),
         subtitle = ifelse( is.null(map_title),      NULL, str_wrap(measure_label))) +
    theme_void() 
...
map_geos <- c("Portland", "Lewiston", "Auburn", "Bangor", "Sanford", "Biddeford")
map_geo_level <- "school"
map_geo_field <- 
...
# Loop across ages
for (a in c("age_0to5")) { # "age_0to2", "age_3to5", "age_0to5", "age_4"
  ests_a <- 
    nowcast_agg_tract %>% 
    filter(age == a,
      ...
      static_map_val(estimates = ests_ag,
                     ...
                     field_desc = 
                       glue("{local_ccdf_name_short} Eligibility - ",
                            "{inc_thresh}% {local_custom_thresh_label}, {age_desc} ",
                            "({date_range})"),
