...
```{r specify and run reaggregations to new age groups}
...
# Replace indications of "kind" in the `age_aggs` input with the calculated month of kindergarten entry

age_aggs <- 
  age_aggs %>% 
  mutate(low_month =  ifelse(low_month  == "kind", end_month_pre_schoolage, low_month) %>% as.numeric(),
         high_month = ifelse(high_month == "kind", end_month_pre_schoolage, high_month) %>% as.numeric())
